...
def query_persisted_cost_summary(root: Path, project_id: str = "") -> Dict[str, Any]:
    store = SQLiteStateStore(root)
    db_path = store.db_path
    if not db_path.is_file():
        return _empty_persisted()
    try:
...
def build_cost_summary(root: Path) -> Dict[str, Any]:
    ...
    return {
        ...
        "persisted": query_persisted_cost_summary(root, project_id=project_id),
