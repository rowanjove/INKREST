...
def chapter_id_from_metadata(meta: Dict[str, Any]) -> str:
    if not meta:
        return ""
    val = meta.get("chapter") or meta.get("chapter_id")
    return normalize_chapter_id_value(val)

def metadata_in_chapter_window(meta: Dict[str, Any], near_chapter_id: Optional[str], window: int) -> bool:
    if not near_chapter_id or window <= 0:
        return True
    try:
        current_id = int(near_chapter_id)
        cid_str = chapter_id_from_metadata(meta)
        if not cid_str:
            return True
        chunk_cid = int(cid_str)
        return abs(chunk_cid - current_id) <= window
    except ValueError:
        return True


class SQLiteEmbeddingVectorStore(VectorStore):
        ...
        if self.chroma_collection is not None:
            try:
                ...
                if embeddings is not None:
                    chroma_payload["embeddings"] = embeddings.tolist()
                self.chroma_collection.upsert(**chroma_payload)
                logger.info("Synced %d chunks to ChromaDB", len(chunks))
            except Exception as e:
                logger.error("Failed to sync chunks to ChromaDB: %s", e)
