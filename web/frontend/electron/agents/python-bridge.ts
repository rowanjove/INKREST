...
export class PythonBridge extends EventEmitter {
  ...
  async runChapter(chapterId: string, goal: string, dryRun: boolean = false): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      const command = this.commandArgs(args);
      ...
      const chapterProcess = this.process;
      let stderrBuffer = '';
      let stdoutBuffer = '';
      let settled = false;
        ...
        try {
          ...
          switch (msg.type) {
            ...
            case 'error':
              ...
              break;
            default:
              this.emit('log', { type: 'log', message: line, level: 'info' });
          }
        } catch {
      ...
      chapterProcess.stdout?.on('data', (data: Buffer) => {
      ...
      chapterProcess.stderr?.on('data', (data: Buffer) => {
      ...
      chapterProcess.on('close', (code) => {
        processLine(stdoutBuffer.trim());
        if (this.process === chapterProcess) this.process = null;
        if (settled) return;
        const error = code !== 0 && code !== null
          ? `Python process exited with code ${code}: ${stderrBuffer.slice(-500)}`
          : 'Python process exited before reporting completion';
        this.emit('error', { type: 'error', error });
        rejectOnce(new Error(error));
      });

      chapterProcess.on('error', (err) => {
        this.emit('error', { type: 'error', error: err.message });
        rejectOnce(err);
      });
    });
